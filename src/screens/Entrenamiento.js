...
function Entrenamiento({ volver }) {
  ...
  const datosGuardados = JSON.parse(
    localStorage.getItem("entreno_" + fecha)
  ) || [];
  ...
  const [entrenoHecho, setEntrenoHecho] = useState(
    JSON.parse(localStorage.getItem("entreno_check_" + fecha)) || false
  );
